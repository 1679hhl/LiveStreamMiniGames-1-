...
namespace DG.Tweening
{
    ...
    public static class ShortcutExtensions
    {
        ...
        {
            if (duration <= 0) {
                if (Debugger.logPriority > 0) Knight.Core.LogManager.LogWarning("DOShakePosition: duration can't be 0, returning NULL without creating a tween");
                return null;
            }
        ...
        {
            if (duration <= 0) {
                if (Debugger.logPriority > 0) Knight.Core.LogManager.LogWarning("DOShakeRotation: duration can't be 0, returning NULL without creating a tween");
                return null;
            }
...
#region Transform Shortcuts
        ...
        /// <returns></returns>
        public static Tweener DOOrthographicSize(this Camera camera, float endValue, float duration, bool snapping = false)
        {
            // 使用委托获取当前的orthographicSize值
            DOGetter<float> getter = () => camera.orthographicSize;
            // 使用委托设置新的orthographicSize值
            DOSetter<float> setter = x => camera.orthographicSize = x;

            // 使用DOTween的To方法创建动画
            TweenerCore<float, float, FloatOptions> tween = DOTween.To(getter, setter, endValue, duration);
            // 设置动画选项，是否平滑移动（这里默认按需求设置为平滑移动，可根据实际调整）
            tween.SetOptions(snapping);
            // 设置动画目标为对应的Camera
        ...
        /// <param name="snapping"></param>
        /// <returns></returns>
        public static Tweener DOMoveRect(this RectTransform rectTransform, Vector2 endPosition, float duration, bool snapping = false)
        {
            // 使用委托获取当前的anchoredPosition
            DOGetter<Vector2> getter = () => rectTransform.anchoredPosition;
            // 使用委托设置新的anchoredPosition
            DOSetter<Vector2> setter = x => rectTransform.anchoredPosition = x;

            // 使用DOTween的To方法创建动画
            TweenerCore<Vector2, Vector2, VectorOptions> tween = DOTween.To(getter, setter, endPosition, duration);
            // 设置动画选项，是否平滑移动
            tween.SetOptions(snapping);
            // 设置动画目标为RectTransform
        ...
        public static Tweener DoRectTransformMoveX(this RectTransform target, float endValue, float duration, bool snapping = false)
        {
            // 使用委托获取当前的anchoredPosition
            DOGetter<Vector2> getter = () => target.anchoredPosition;
            // 使用委托设置新的anchoredPosition
            DOSetter<Vector2> setter = x => target.anchoredPosition = x;
            // 创建目标位置的Vector2，只改变x轴的值
            Vector2 endPosition = new Vector2(endValue, target.anchoredPosition.y);
            // 使用DOTween的To方法创建动画
            TweenerCore<Vector2, Vector2, VectorOptions> tween = DOTween.To(getter, setter, endPosition, duration);
            // 设置动画选项，约束为X轴，是否平滑移动
            tween.SetOptions(AxisConstraint.X, snapping);
            // 设置动画目标为RectTransform
        ...
        /// 改变 Image 的透明度
        /// </summary>
        /// <param name="target">要修改透明度的 Image 组件</param>
        /// <param name="endAlphaValue">目标透明度值（范围为 0 到 1）</param>
        /// <param name="duration">动画持续时间</param>
        /// <returns>生成的 Tweener 对象，可用于进一步操作动画</returns>
        public static Tweener DoImageAlphaChange(this Image target, float endAlphaValue, float duration)
        {
            // 使用委托获取当前的颜色透明度（alpha 值）
            DOGetter<float> getter = () => target.color.a;
            // 使用委托设置新的颜色透明度（alpha 值）
            DOSetter<float> setter = alpha =>
            {
            ...
            // 创建目标透明度值
            float endAlpha = endAlphaValue;
            // 使用 DOTween 的 To 方法创建动画，这里使用 FloatOptions 来正确配置动画相关选项
            TweenerCore<float, float, FloatOptions> tween = DOTween.To(() => target.color.a, alpha =>
            {
                Color currentColor = target.color;
                currentColor.a = alpha;
                target.color = currentColor;
        ...
        /// <summary>
        /// 为 TMP_Text 组件创建一个从起始百分比值过渡到目标百分比值的动画
        /// </summary>
        /// <param name="textComponent">要更新的 TMP_Text 组件</param>
        /// <param name="startValue">起始百分比值（范围 0 - 1）</param>
        /// <param name="endValue">目标百分比值（范围 0 - 1）</param>
        /// <param name="duration">动画持续时间</param>
        public static Tweener DOPercentageChange(this TMP_Text textComponent, float startValue, float endValue, float duration)
        {
            return DOTween.To(() => startValue, x =>
            {
                // 更新 TMP_Text 组件的文本显示
                textComponent.text = (x * 100).ToString("F2", CultureInfo.InvariantCulture) + "%";
            }, endValue, duration);
        }
        /// <summary>
        /// 为 TMP_Text 组件创建一个从起始百分比值过渡到目标百分比值的动画
        ...
        /// <param name="endValue">目标百分比值（范围 0 - 1）</param>
        /// <param name="duration">动画持续时间</param>
        public static Tweener DOFloatChange(this TMP_Text textComponent, float startValue, float endValue, float duration)
        {
            return DOTween.To(() => startValue, x =>
            {
                // 更新 TMP_Text 组件的文本显示

                textComponent.text = NumToString(x);
            }, endValue, duration);
        }
        ...
        /// <param name="duration">动画持续时间</param>
        /// <returns>生成的 Tweener 对象，可用于进一步操作动画</returns>
        public static Tweener DoITextMeshProAlphaChange(this TMP_Text target, float endAlphaValue, float duration)
        {
            // 使用委托获取当前的颜色透明度（alpha 值）
            DOGetter<float> getter = () => target.color.a;
            // 使用委托设置新的颜色透明度（alpha 值）
        ...
        /// <param name="duration">动画持续时间</param>
        /// <returns>生成的 Tweener 对象，可用于进一步操作动画</returns>
        public static Tweener DoDisplayUGUIAlphaChange(this DisplayUGUI target, float endAlphaValue, float duration)
        {
            // 使用委托获取当前的颜色透明度（alpha 值）
            DOGetter<float> getter = () => target.color.a;
            // 使用委托设置新的颜色透明度（alpha 值）
        ...
        /// <param name="duration"></param>
        /// <returns></returns>
        public static Tweener DoSpriteRendererAlphaChange(this SpriteRenderer target, float endAlphaValue, float duration)
        {
            // 使用委托获取当前的颜色透明度（alpha值）
            DOGetter<float> getter = () => target.color.a;
            // 使用委托设置新的颜色透明度（alpha值）
            DOSetter<float> setter = alpha =>
            {
                Color currentColor = target.color;
                currentColor.a = alpha;
            ...
            // 创建目标透明度值
            float endAlpha = endAlphaValue;
            // 使用DOTween的To方法创建动画，这里使用FloatOptions来正确配置动画相关选项
            TweenerCore<float, float, FloatOptions> tween = DOTween.To(() => target.color.a, alpha =>
            {
                Color currentColor = target.color;
                currentColor.a = alpha;
        ...
        {
            if (duration <= 0) {
                if (Debugger.logPriority > 0) Knight.Core.LogManager.LogWarning("DOPunchPosition: duration can't be 0, returning NULL without creating a tween");
                return null;
            }
        ...
        {
            if (duration <= 0) {
                if (Debugger.logPriority > 0) Knight.Core.LogManager.LogWarning("DOPunchScale: duration can't be 0, returning NULL without creating a tween");
                return null;
            }
        ...
        {
            if (duration <= 0) {
                if (Debugger.logPriority > 0) Knight.Core.LogManager.LogWarning("DOPunchRotation: duration can't be 0, returning NULL without creating a tween");
                return null;
            }
        ...
        {
            if (duration <= 0) {
                Knight.Core.LogManager.Log(Debugger.logPriority);
                if (Debugger.logPriority > 0) Knight.Core.LogManager.LogWarning("DOShakeScale: duration can't be 0, returning NULL without creating a tween");
                return null;
            }
...
//            Quaternion to = target.localRotation;
        ...
        {
            if (duration <= 0) {
                if (Debugger.logPriority > 0) Knight.Core.LogManager.LogWarning("DOBlendablePunchRotation: duration can't be 0, returning NULL without creating a tween");
                return null;
            }
            Vector3 to = Vector3.zero;
