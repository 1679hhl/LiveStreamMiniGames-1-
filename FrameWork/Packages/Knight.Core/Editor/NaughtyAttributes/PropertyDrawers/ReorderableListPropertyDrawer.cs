                ...
                if (!this.reorderableListsByPropertyName.ContainsKey(property.name))
                {
                    ReorderableList reorderableList = new ReorderableList(property.serializedObject, property, true, true, true, true)
                    {
                        ...
                        drawElementCallback = (Rect rect, int index, bool isActive, bool isFocused) =>
                        {
                            ...
                            EditorGUI.PropertyField(new Rect(rect.x, rect.y, rect.width/2, EditorGUIUtility.singleLineHeight), element);
                            EditorGUI.LabelField(new Rect(rect.x + rect.width / 2, rect.y, rect.width / 2, EditorGUIUtility.singleLineHeight), element.objectReferenceValue.ToString());
                        }
                    };
