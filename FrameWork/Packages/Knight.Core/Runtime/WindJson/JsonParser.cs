...
namespace Knight.Core.WindJson
{
    ...
    public class JsonParser
    {
        ...
        public string PretreatmentProc()
        {
            ...
            {
                //跳过那些无用的字符 ' ', '\t', '\r' '\n'
                if (LexicalAnalysis.isSpecialSymbol(originData, i, ref end))
                {
                    i = end;
                    continue;
                }
                ...
                {
                    //Knight.Core.LogManager.Log(tempWord);
                    i = end;
                    continue;
                }
                temp += originData[i];
        ...
        public JsonNode Parser()
        {
            ...
            {
                //跳过那些无用的字符 ' ', '\t', '\r' '\n' 注释
                if (LexicalAnalysis.isSpecialSymbol(originData, i, ref end) || !string.IsNullOrEmpty(LexicalAnalysis.isComment(originData, i, ref end)))
                {
                    i = end;
                    continue;
                }
                ...
                if (rCurSymbol != null)
                {
                    switch (rCurSymbol.type)
                    {
                        case JsonSymbolType.Unknown:
                            this.isValid = false;
                            Knight.Core.LogManager.LogError("Json 未知类型 格式不匹配!  " + this.originData);
                            return null;
                        ...
                        case JsonSymbolType.ObjEnd:
                            ...
                            {
                                this.isValid = false;
                                Knight.Core.LogManager.LogError("{} 大括号不匹配!  " + this.originData);
                                return null;
                            }
                            rNodeStack.Pop();
                        ...
                        case JsonSymbolType.ArrayEnd:
                            ...
                            while (rNodeStack.Count != 0 && rNodeStack.Peek().type != JsonSymbolType.ArrayStart)
                            {
                                var rTopSymbol = rNodeStack.Pop();
                                if (rTopSymbol.type == JsonSymbolType.ObjSplit)
                                {
                            ...
                            {
                                this.isValid = false;
                                Knight.Core.LogManager.LogError("[] 中括号不匹配!  " + this.originData);
                                return null;
                            }
                            rNodeStack.Pop();
        ...
        private void Generate_ElementSymbol(ref Stack<JsonSymbolItem> rNodeStack, JsonSymbolItem rCurSymbol)
        {
            ...
            if (rSymbol1.type == JsonSymbolType.ObjSplit || rSymbol1.type == JsonSymbolType.ArrayStart)
            {
                ...
                if (rCurSymbol.type != JsonSymbolType.Null)
            ...
            {
                Knight.Core.LogManager.LogError($"Json grammar error! Json: {this.originData}.");
                this.isValid = false;
            }
        }
        ...
        private JsonSymbolItem buildSymbolItem(JsonSymbolItem rLastSymbol, int begin, ref int end)
        {
            ...
            else if ((originData[begin] == 'N' || originData[begin] == 'n') && originData[begin + 1] == 'u' && originData[begin + 2] == 'l' && originData[begin + 3] == 'l')
            {
                end = begin + 4;
                return new JsonSymbolItem() { type = JsonSymbolType.Null };
            }
            ...
            if (!string.IsNullOrEmpty(tempWord = LexicalAnalysis.isKeyword(originData, begin, ref end)))
            {
                ...
                LexicalAnalysis.parseSpecialSymbol(originData, end, ref end);
                if (originData[end] == ':')
                {
            ...
            if (LexicalAnalysis.getString(originData, ref tempWord, begin, ref end))
            {
                if (LexicalAnalysis.NeedUnBackslashSymbol(tempWord))
                    tempWord = LexicalAnalysis.UnBackslashSymbol(tempWord);
                JsonSymbolItem rSymbol = new JsonSymbolItem() { value = tempWord, type = JsonSymbolType.Value, node = new JsonData(tempWord) };
                LexicalAnalysis.parseSpecialSymbol(originData, end, ref end);
                if (originData[end] == ':')
                {
            ...
            //Knight.Core.LogManager.Log(string.Format("Json parse symbol item error! LastSymbol = {0}",
        ...
        public static JsonNode ToJsonNode(object rObject)
        {
            ...
            else if (rType.IsPrimitive) //如果是实例
            {
                if (rType == typeof(int))
                    ...
                    Knight.Core.LogManager.LogError(string.Format("Type = {0}, 不支持序列化的变量类型!", rObject.GetType()));
            }
