...
namespace UnityEditor.UI
{
    ...
    public class ViewModelContainerInspector : InspectorEditor
    {
        ...
        private List<MemberBindingAbstract> mMemberBindings = new List<MemberBindingAbstract>();
        private List<ViewModelDataSourceTemplate> mViewModelDataSourceTemplates = new List<ViewModelDataSourceTemplate>();
        ...
        public override void OnInspectorGUI()
        {
            ...
            if (GUILayout.Button("查找属性绑定的节点", GUILayout.ExpandWidth(true)))
            {
                this.FindPropertyBindingNode();
            }
            ...
            foreach (var rBinding in mViewModelDataSourceTemplates)
            {
                EditorGUILayout.ObjectField(rBinding, rBinding.GetType(), false);
            }
        ...
        private int CheckBindings()
        {
            ...
            var rMemberBindings = this.mTarget.GetComponentsInChildren<MemberBindingAbstract>(true);
            var rViewModelDataSourceTemplates = this.mTarget.GetComponentsInChildren<ViewModelDataSourceTemplate>(true);
            ...
            this.mViewModelDataSourceTemplates.Clear();
            foreach (var rBinding in rViewModelDataSourceTemplates)
            {
                rBinding.GetPaths();
                if (rBinding.IsSelectionValid() == false)
                {
                    this.mViewModelDataSourceTemplates.Add(rBinding);
                    nInvalidCount++;
                }
            }
        ...
        public void FindPropertyBindingNode()
        {
            if (InputDialog.Show("输入需要查找的属性名称", out var rResults, new string[] { "" }, "属性名称:"))
            {
                var rPropertyName = rResults[0];
                var rMemberBindingAbstracts = this.mTarget.MemberBindingAbstracts;
                if (rMemberBindingAbstracts != null)
                {
                    foreach (var rMemberBindingAbstract in rMemberBindingAbstracts)
                    {
                        if (rMemberBindingAbstract.ViewModelPropName.IndexOf(rPropertyName, StringComparison.OrdinalIgnoreCase) >= 0)
                        {
                            Debug.Log($"ViewPath:{rMemberBindingAbstract.ViewPath}", rMemberBindingAbstract.transform);
                        }
                    }
                }
            }
