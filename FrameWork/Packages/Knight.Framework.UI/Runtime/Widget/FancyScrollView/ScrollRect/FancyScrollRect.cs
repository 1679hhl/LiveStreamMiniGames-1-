...
namespace FancyScrollView
{
        ...
        where TContext : class, IFancyScrollRectContext, new()
    {
        ...
        protected override void UpdateContents(int nItemsCount)
        {
            AdjustCellIntervalAndScrollOffset();

            base.UpdateContents(nItemsCount);

            Scroller.SetTotalCount(nItemsCount);
            RefreshScroller();
        }
        ...
        protected virtual void OnValidate()
        {
            ...
            {
                loop = false;
                Knight.Core.LogManager.LogError("Loop is currently not supported in FancyScrollRect.");
            }

            if (Scroller.SnapEnabled)
            {
                Scroller.SnapEnabled = false;
                Knight.Core.LogManager.LogError("Snap is currently not supported in FancyScrollRect.");
            }

            if (Scroller.MovementType == MovementType.Unrestricted)
            {
                Scroller.MovementType = MovementType.Elastic;
                Knight.Core.LogManager.LogError("MovementType.Unrestricted is currently not supported in FancyScrollRect.");
            }
        }
    }
