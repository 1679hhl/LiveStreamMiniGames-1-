...
namespace FancyScrollView
{
    ...
    public abstract class FancyScrollView<TItemData, TContext> : MonoBehaviour where TContext : class, new()
    {
        ...
        /// 更新每个Cell的内容
        /// </summary>
        public Action<ICell> OnUpdateContentAction;
        public Action<ICell> OnUpdateContentAction_Other;
        ...
        public IList<FancyCell<TItemData, TContext>> Pool => this.pool;
        ...
        protected virtual void UpdateContents(int nItemsCount)
        {
            if(this.ItemsSource != null && this.ItemsSource.Count == nItemsCount)
            {
                this.Refresh();
            }
            else
            {
                var rItemArray = new TItemData[nItemsCount];
                var rItemSource = new List<TItemData>(rItemArray);
        ...
        void UpdateCells(float firstPosition, int firstIndex, bool forceRefresh)
        {
            for (var i = 0; i < pool.Count; i++)
            {
                ...
                if (forceRefresh || cell.Index != index || !cell.IsVisible)
                {
                    ...
                    var rCellType = cell.GetType();

                    if (rCellType == typeof(FancyBindingGridView.CellGroup))
                    {
                        var rCellGroup = cell as FancyBindingGridView.CellGroup;
                        var rCellArray = rCellGroup.CellsArray;

                        for (int j = 0; j < rCellArray.Length; j++)
                        {
                            var rGroupCell = rCellArray[j];
                            if(rGroupCell != null)
                            {
                                this.OnUpdateContentAction?.Invoke(rGroupCell);
                                this.OnUpdateContentAction_Other?.Invoke(rGroupCell);
                            }
                        }
                    }
                    else
                    {
                        this.OnUpdateContentAction?.Invoke(cell);
                        this.OnUpdateContentAction_Other?.Invoke(cell);
                    }
                }
