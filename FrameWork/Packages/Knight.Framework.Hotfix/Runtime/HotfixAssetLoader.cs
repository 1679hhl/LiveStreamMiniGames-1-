//======================================================================
//        Copyright (C) 2015-2020 Winddy He. All rights reserved
//        Email: hgplan@126.com
//======================================================================
using System.Collections;
using System.Collections.Generic;
using UnityEngine;
using Knight.Core;
using System.IO;
using System.Threading.Tasks;
using UnityFx.Async;

namespace Knight.Framework.Hotfix
{
    public class HotfixLoaderRequest
    {
        public string ABPath;
        public string ModuleName;

        public byte[] DllBytes;
        public byte[] PdbBytes;

        public string DLLPath;
        public string PDBPath;

        public HotfixLoaderRequest(string rABName, string rModuleName)
        {
            this.ABPath = rABName;
            this.ModuleName = rModuleName;
        }
    }

    public class HotfixAssetLoader : TSingleton<HotfixAssetLoader>
    {
        private HotfixAssetLoader() { }

        public async Task<HotfixLoaderRequest> Load(string rABPath, string rHotfixModuleName)
        {
            var rRequest = new HotfixLoaderRequest(rABPath, rHotfixModuleName);

            string rDLLPath = HotfixManager.HotfixDllDir + rRequest.ModuleName + ".bytes";
            string rPDBPath = HotfixManager.HotfixDllDir + rRequest.ModuleName + ".PDB.bytes";
            
#if UNITY_EDITOR
            // 编辑器下的HotfixDebug模式直接加载DLL文件，方便做断点调试
            rDLLPath = HotfixManager.HotfixDebugDllDir + $"{rHotfixModuleName}.dll";
            rPDBPath = HotfixManager.HotfixDebugDllDir + $"{rHotfixModuleName}.pdb";
            Knight.Core.LogManager.Log("---Simulate load hotfix dll: " + rDLLPath);
            rRequest.DLLPath = rDLLPath;
            rRequest.PDBPath = rPDBPath;
            return rRequest;
#endif
            
#if UNITY_EDITOR
            // 编辑器下的HotfixDebug模式直接加载DLL文件，方便做断点调试
            if (HotfixManager.Instance.IsHotfixDebugMode())
            {
                // Game.Hotfix
                rDLLPath = HotfixManager.HotfixDebugDllDir + $"{rHotfixModuleName}.dll";
                rPDBPath = HotfixManager.HotfixDebugDllDir + $"{rHotfixModuleName}.pdb";
                Knight.Core.LogManager.Log("---Simulate load hotfix dll: " + rDLLPath);
                rRequest.DLLPath = rDLLPath;
                rRequest.PDBPath = rPDBPath;
                return rRequest;
            }
#endif

            if (AssetLoader.Instance.IsSumilateMode_Script())
            {
#if HOTFIX_HYBRIDCLR_USE
                rDLLPath = $"Library/ScriptAssemblies/{rHotfixModuleName}.dll";
                rPDBPath = $"Library/ScriptAssemblies/{rHotfixModuleName}.pdb";
                Knight.Core.LogManager.Log("---HybridCLR mode simulate load ab: " + rDLLPath);
                rRequest.DLLPath = rDLLPath;
                rRequest.PDBPath = rPDBPath;
#else
                Knight.Core.LogManager.Log("---Simulate load ab: " + rRequest.ABPath);
                rRequest.DllBytes = File.ReadAllBytes(rDLLPath);
                rRequest.PdbBytes = File.ReadAllBytes(rPDBPath);
#endif
            }
            else
            {
                var rDLLAssetRequest = AssetLoader.Instance.LoadAssetAsync(rRequest.ABPath, rDLLPath, false);
                await rDLLAssetRequest.Wait();
                if (rDLLAssetRequest.Asset != null)
                {
                    var rTextAsset = rDLLAssetRequest.Asset as TextAsset;
                    if (rTextAsset != null)
                        rRequest.DllBytes = rTextAsset.bytes;
                }
                AssetLoader.Instance.UnloadAsset(rDLLAssetRequest);

                var rPDBAssetRequest = AssetLoader.Instance.LoadAssetAsync(rRequest.ABPath, rPDBPath, false);
                await rPDBAssetRequest.Wait();
                if (rPDBAssetRequest.Asset != null)
                {
                    var rTextAsset = rPDBAssetRequest.Asset as TextAsset;
                    if (rTextAsset != null)
                        rRequest.PdbBytes = rTextAsset.bytes;
                }
                AssetLoader.Instance.UnloadAsset(rPDBAssetRequest);
            }

            return rRequest;
        }
    }
}
