...
namespace Knight.Hotfix.Core
{
	...
	public class HotfixTypeSearchAttribute : Attribute
	{
        ...
        public List<string> TypeFullNames
        {
            get
            {
                return HotfixTypeSearchBase.GetTypeFullNames(this.TypeSearchType);
            }
        }
	}
    /// <summary>
    ...
    public class HotfixTypeSearchCore : THotfixSingleton<HotfixTypeSearchCore>
    {
        ...
        {
            if (!this.mSearchTypes.ContainsKey(rType))
                return new List<Type>();

            return (List<Type>)this.mSearchTypes[rType];
        }
        ...
        private HotfixTypeSearchCore()
        {
            ...
            foreach (var rType in TypeResolveManager.Instance.GetTypes("Game.Hotfix"))
            {
                ...
                {
                    var rSearchType = this.GetNoPublicField<Type>(rType.HotfixSearchBaseTo(typeof(HotfixTypeSearchBase)), "_Type");
                    var rIgnoreType = this.GetNoPublicField<Type>(rType.HotfixSearchBaseTo(typeof(HotfixTypeSearchBase)), "_IgnoreAttributeType");
            ...
            foreach (var rType in TypeResolveManager.Instance.GetTypes("Game.Hotfix"))
            {
                foreach (var rTypeSearchSubClass in rTypeSearchSubClasses)
                {
                    ...
                    {
                        var rTypeList = this.ReceiveTypeList(rTypeSearchSubClass.Key);
        ...
        {
            if (!this.mSearchTypes.ContainsKey(rType))
                this.mSearchTypes.Add(rType, new List<Type>());
            return (List<Type>)this.mSearchTypes[rType];
        }
    ...
    ///     不支持抽象类
    /// </summary>
    public class HotfixTypeSearchFull<TSearchType, TIgnoreType> : HotfixTypeSearchBase where TIgnoreType : System.Attribute
    {
