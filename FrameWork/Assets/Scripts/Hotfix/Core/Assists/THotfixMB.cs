            ...
            for (int i = 0; i < rFiledInfos.Length; i++)
            {
                ...
                if (rBindingAttr != null)
                {
                    ...
                    if (rUnityObject == null)
                        Knight.Core.LogManager.LogErrorFormat("Not find binding data, please check prefab and hofix script. {0}", rFiledInfos[i].Name);
                    else
                    {
                        if (!rUnityObject.Type.Equals(rFiledInfos[i].FieldType.FullName))
                            Knight.Core.LogManager.LogErrorFormat("Binding data type is not match. {0}", rFiledInfos[i].Name);
                        else
