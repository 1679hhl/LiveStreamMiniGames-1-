...
public class Reporter : MonoBehaviour
{
    ...
    bool curFocusState = true;
    bool showDebugConsoleButton = false;
...
#endif
        width = Display.main.systemWidth / 2;
        height = 0;
    }
    ...
    float continuousClickLimitTime = 2f;
    float lastContinuousClickTime = 0f;
    int continuousClickCount = 5;
    int curContinuousClickCount = 0;
    int continuousClickTouchCount = 3;
    bool continuousClickTouchPress = false;
    bool isContinuousClickDone()
    {
// #if !GAME_DEVELOPE && !UNITY_EDITOR
//         return false;
// #endif
//         if (Application.platform == RuntimePlatform.Android ||
//             Application.platform == RuntimePlatform.IPhonePlayer)
//         {
//             if (Time.time - lastContinuousClickTime > continuousClickLimitTime)
//             {
//                 curContinuousClickCount = 0;
//             }
//             if (Input.touchCount == continuousClickTouchCount && !continuousClickTouchPress)
//             {
//                 curContinuousClickCount++;
//                 continuousClickTouchPress = true;
//                 lastContinuousClickTime = Time.time;
//             }
//             if (Input.touchCount == 0)
//             {
//                 continuousClickTouchPress = false;
//             }
//             if (curContinuousClickCount >= continuousClickCount)
//             {
//                 curContinuousClickCount = 0;
//                 return true;
//             }
...
//         return false;
    ...
    private void OnApplicationFocus(bool focus)
    {
        ...
        if (Application.platform == RuntimePlatform.Android ||
            Application.platform == RuntimePlatform.IPhonePlayer)
        {
            if (!curFocusState && focus)
            {
                showDebugConsoleButton = !showDebugConsoleButton;
            }
            curFocusState = focus;
            width = Display.main.systemWidth / 2;
            height = 0;
    ...
    private void OnGUI()
    {
        if (showDebugConsoleButton && !show)
        {
            if (GUI.Button(new Rect(width - 60, height, 120, 60), "控制台"))
            {
                doShow();
            }
        }
